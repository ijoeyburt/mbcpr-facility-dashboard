...
function wirePushToTalk(buttonEl, zone = 'both') {
  const getZone = typeof zone === 'function' ? zone : () => zone;
  const start = (e) => {
    e.preventDefault();
    buttonEl.classList.add('talking');
    startTalking(getZone());
  };
  const end = (e) => {
    e.preventDefault();
...
function formatDue(task) {
  ...
  if (task.cadence === 'daily') {
    if (task.overdue || !task.lastCompleted) return 'Not done yet today';
    return `Done today at ${new Date(task.lastCompleted).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}`;
  }
  ...
  tasks
    ...
    .forEach((task) => {
      ...
      card.innerHTML = `
        ...
        ${showHistory ? '<div class="history-link" style="font-size:11px;color:var(--accent);margin-top:6px;">View responses</div>' : ''}`;
      card.addEventListener('click', (e) => {
        if (showHistory && e.target.classList.contains('history-link')) {
          e.stopPropagation();
...
function formatHistoryEntry(task, values) {
  let fields = [];
  if (task.type === 'checklist') fields = task.fields;
  else if (task.type === 'numeric') fields = task.groups.flatMap((g) => g.fields);
  else if (task.type === 'report') fields = task.fields;

  return fields
    .map((f) => {
      const raw = values[f.key];
      let display;
      if (f.type === 'text' || task.type === 'numeric' || task.type === 'report') {
        display = raw || '-';
      } else {
        const initials = values[f.key + '_initials'];
        display = raw ? `Checked${initials ? ' (' + initials + ')' : ''}` : 'Unchecked';
      }
      return `<div style="font-size:13px;margin-bottom:4px;"><span style="color:var(--text-secondary);">${f.label}:</span> ${display}</div>`;
    })
    .join('');
}

async function openTaskHistoryPopup(task) {
  ...
  const entriesHtml = entries.length
    ? entries
        .map((entry) => {
          const values = entry.values || entry; // report endpoints return the record itself
          const at = entry.at || entry.at === 0 ? entry.at : null;
          return `<div class="log-row">
            <p class="log-meta" style="margin:0 0 6px;">${at ? new Date(at).toLocaleString() : ''}</p>
            ${formatHistoryEntry(task, values)}
          </div>`;
        })
        .join('')
    : '<p style="font-size:13px;color:var(--text-muted);">No responses yet.</p>';

  overlay.innerHTML = `
    <div class="modal">
      <div class="modal-header">
        <p class="modal-title">${task.title} — responses</p>
        <button class="close-btn" aria-label="Close">&times;</button>
      </div>
      <div class="log-list" style="padding:0 1.1rem;">${entriesHtml}</div>
    </div>`;
  document.body.appendChild(overlay);
  overlay.querySelector('.close-btn').addEventListener('click', () => overlay.remove());
}
